...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request == null && response2 == null) {
            return new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(IInputController.KEYCODE_KNOB_TEMP_UP).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1L).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
        if (request == null) {
    ...
    static boolean isContentSpecificHeader(String str) {
        return com.alibaba.sdk.android.oss.common.utils.HttpHeaders.CONTENT_LENGTH.equalsIgnoreCase(str) || com.alibaba.sdk.android.oss.common.utils.HttpHeaders.CONTENT_ENCODING.equalsIgnoreCase(str) || "Content-Type".equalsIgnoreCase(str);
    }
}
