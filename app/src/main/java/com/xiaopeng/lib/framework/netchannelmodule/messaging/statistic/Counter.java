...
public class Counter {
    ...
    public void tryToCommitData(boolean z) {
        ...
        if (z || j3 >= MAX_VALUE_THRESHOLD || this.mLastDate.get() < currentTimeMillis) {
            ...
            if (this.mNeedToPost && j + j2 > MIN_COMMIT_COUNT && !c.pq()) {
                IDataLog iDataLog = (IDataLog) Module.get(a.class).get(IDataLog.class);
                iDataLog.sendStatData(iDataLog.buildMoleEvent().setEvent(EVENT_NAME).setPageId(GlobalConfig.MOLE_PAGE_ID).setButtonId(MOLE_TRAFFIC_MQTT_BUTTON_ID).setProperty(STAT_KEY_PACKAGE_NAME, GlobalConfig.getApplicationSimpleName()).setProperty(STAT_KEY_TRY_PUBLISH, Long.valueOf(j)).setProperty("fail", Long.valueOf(j5)).setProperty(STAT_KEY_PUBLISHED, Long.valueOf(j4)).setProperty(STAT_KEY_PUBLISHED_SIZE_KB, Long.valueOf(j3 >> MIN_COMMIT_COUNT)).setProperty(STAT_KEY_RECEIVED, Long.valueOf(j2)).setProperty(STAT_KEY_INIT_FAILED, Long.valueOf(j6)).setProperty(STAT_KEY_LAST_DATE, Long.valueOf(j7)).setProperty(STAT_KEY_NETWORK_TYPE, Integer.valueOf(GlobalConfig.getNetworkType())).build());
            }
        }
    }
