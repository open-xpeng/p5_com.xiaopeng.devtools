...
public class TracingInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (!isXiaopengHost(url.host())) {
            c.f(TAG, "isXiaopengHost(url.host()) == false, host:" + url.host());
        ...
        c.f(TAG, "Tracing intercept operationName:" + substring + " traceId:" + hexString + " spanId:" + hexString3 + " sampled:" + sampled);
        try {
            ...
            c.f(TAG, "Tracing intercept finish, code:" + code);
            return proceed;
        } catch (Exception e) {
            ...
            c.f(TAG, "Tracing intercept finish, code:-1");
            throw e;
        }
    }
    ...
    private void decorateAndFinishSpan(Span span, String str, String str2, HttpUrl httpUrl, int i, Exception exc) {
        span.name(str).kind(Span.Kind.CLIENT).tag(TAG_KEY_HOST, httpUrl.host()).tag(TAG_KEY_METHOD, str2).tag(TAG_KEY_PATH, httpUrl.encodedPath()).tag(TAG_KEY_URL, httpUrl.toString()).tag(TAG_KEY_STATUS, String.valueOf(i)).tag(TAG_KEY_VID, String.valueOf(i.lp())).tag(TAG_KEY_VIN, i.eH()).tag(TAG_KEY_CDUID, i.getHardwareId()).tag(TAG_KEY_SID, i.oS()).tag(TAG_KEY_NET, String.valueOf(e.as(GlobalConfig.getApplicationContext()))).error(exc).finish();
    }
}
